...
/// <summary>Scatters ground cover - short grass and flowers - over exposed grass. It runs after the tree
/// decorator and reads the finished cube directly: a plant goes one block above any grass_block whose cell above
/// is open, so it never lands on sand, stone, water, or beneath a tree's canopy. Each plant is one cell, so it
/// never crosses a cube border - only this cube's own columns are considered. Placement is a per-column hash
/// scaled by the biome's grass/flower density; flowers also fold in the feature-density map, forming patches.</summary>
    ...
    public void Decorate(Chunk chunk, Vector3i cube) {
        ...
        for (int lz = 0; lz < Chunk.Size; lz++)
            for (int lx = 0; lx < Chunk.Size; lx++) {
                ...
                if (gy < 0 || gy >= top) continue;                      // empty, or surface at the cube's ceiling
                if (chunk.GetBlock(lx, gy, lz) != grassBlock) continue; // only on grass (skips sand/leaves/water)
